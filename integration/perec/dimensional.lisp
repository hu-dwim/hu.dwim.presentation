...
(def (component e) time-provider (component-environment/abstract content/mixin)
  ((time :type hu.dwim.perec::timestamp)))

(def component-environment time-provider
  (hu.dwim.perec::call-with-time (time-of -self-) #'call-next-method))
  ...
  (bind (((:read-only-slots single) component))
    (if single
        (bind ((partial-timestamp-string (princ-to-string (component-value-of (range-of component)))))
          (list (hu.dwim.perec::first-moment-for-partial-timestamp partial-timestamp-string)
                (hu.dwim.perec::last-moment-for-partial-timestamp partial-timestamp-string)))
        (not-yet-implemented))))
...
(def component-environment validity-provider
  (bind ((year (component-value-of (selector-of -self-))))
    (hu.dwim.perec::call-with-validity-range (local-time:encode-timestamp 0 0 0 0 1 1 year :offset 0)
...
(def (macro e) coordinates-provider (dimensions coordinates &body content)
  `(make-instance 'coordinates-provider
                  :dimensions (mapcar #'hu.dwim.perec:lookup-dimension ,dimensions)
...
(def component-environment coordinates-provider
  (bind ((dimensions (dimensions-of -self-))
    ...
    (hu.dwim.perec:with-coordinates dimensions coordinates
      (with-error-log-decorator (make-error-log-decorator
                                  ...
                                  (iter (for dimension :in dimensions)
                                        ...
                                        (format t "~%  ~S: ~@<~A~:>" (hu.dwim.perec:name-of dimension) coordinate)))
        (call-next-method)))))
...
(def constructor coordinates-dependent-mixin ()
  (bind (((:slots dimensions coordinates) -self-))
    (setf dimensions (mapcar 'hu.dwim.perec:lookup-dimension dimensions)
          coordinates (hu.dwim.perec:make-empty-coordinates dimensions))))
...
(def render-xhtml :before coordinates-dependent-mixin ()
  (setf (coordinates-of -self-)
        (iter (for dimension :in (dimensions-of -self-))
              ...
              (for new-coordinate = (hu.dwim.perec:coordinate dimension))
              (unless (hu.dwim.perec:coordinate-equal dimension (ensure-list old-coordinate) (ensure-list new-coordinate))
...
(def (function e) coordinates-bound-according-to-dimension-type-p (component)
  (iter (for dimension :in (dimensions-of component))
        (always (etypecase dimension
                  (hu.dwim.perec::inheriting-dimension #t)
                  (hu.dwim.perec::ordering-dimension #t)
                  (hu.dwim.perec::dimension
                   (typep (hu.dwim.perec::coordinate dimension) (hu.dwim.perec::the-type-of dimension)))))))
...
(def layered-method make-alternatives ((component d-value-inspector) (class standard-class) (prototype hu.dwim.perec::d-value) (instance hu.dwim.perec::d-value))
...
(def refresh d-value-table-inspector
  (bind (((:slots instance columns rows) -self-)
         (dimensions (hu.dwim.perec::dimensions-of instance)))
...
(def refresh d-value-row-inspector
  (bind (((:slots value coordinates instance cells) -self-))
    (setf cells (cons (make-viewer value)
                      (mapcar [make-coordinate-inspector -self- !1 !2] (hu.dwim.perec::dimensions-of instance) coordinates)))))
  ...
  (bind (((:read-only-slots instance) self))
    (iter (for (axes-slot-name . dimension-names) :in (collect-pivot-table-dimension-axes-groups self (class-of instance) instance))
          (dolist (dimension-name dimension-names)
            (bind ((dimension (hu.dwim.perec::lookup-dimension dimension-name)))
...
(def (generic e) collect-pivot-table-dimension-axes-groups (component class instance)
  (:method ((component d-value-pivot-table-component) (class standard-class) (instance hu.dwim.perec::d-value))
...
(def function make-d-value-pivot-table-cell (component sheet-path row-path column-path d-value)
  (bind ((path (append sheet-path row-path column-path))
         (dimensions (hu.dwim.perec::dimensions-of d-value))
         (cell-dimensions nil)
         (coordinates (iter (for dimension :in dimensions)
                            (collect (aif (find dimension path :key #'dimension-of)
                                          ...
                                          (progn
                                            ...
                                            hu.dwim.perec::+whole-domain-marker+)))))
         (value (hu.dwim.perec::value-at-coordinates d-value coordinates)))
    ;; TODO: add type to d-value and pass it down
    (if (hu.dwim.perec::single-d-value-p value)
        (aif (hu.dwim.perec::single-d-value value)
             (make-viewer it)
             (empty))
        (if (hu.dwim.perec::empty-d-value-p value)
            (empty)
            ;; TODO: is this remove-dimensions call dangerous?
            (make-viewer (hu.dwim.perec::remove-dimensions value (set-difference dimensions cell-dimensions))
...
(def (component e) pivot-table-dimension-axis-component (abstract-d-value-component
  ...
  ((dimension :type hu.dwim.perec::dimension)))
  ...
  (bind (((:slots dimension categories instance) -self-))
    (setf categories
          (mapcar (lambda (coordinate)
                  ...
                  (hu.dwim.perec::d-value-dimension-coordinate-list instance dimension :mode :intersection)))))
...
(def method make-reference-label ((reference reference-component) (class standard-class) (instance hu.dwim.perec::dimension))
