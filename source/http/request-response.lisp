  ...
  (disallow-response-caching
   (bind ((*print-pretty* #f)
     ...
     (make-byte-vector-response ((+header/content-type+ +utf-8-plain-text-content-type+))
       (format t "Time: ~A~%" (local-time:format-timestring nil (local-time:now)))
       ;;(format t "HTTP server uptime: ~A~%" (format-timestring nil ()))
       (format t "Total number of requests processed: ~A~%" (processed-request-count-of *server*))
       (format t "Number of failed requests: ~A~%" (failed-request-count-of *server*))
       (format t "Number of requests to valid sessions: ~A~%" (number-of-requests-to-sessions-of *application*))
       (format t "Heap usage: ~,2F MB~%" (/ (sb-kernel::dynamic-usage) 1024d0 1024d0))
       ;;(format t "Maximum heap size: ~,2F MB~%" (/ (sb-kernel::dynamic-) 1024d0 1024d0))
       (format t "Number of web sessions: ~A~%" (iter (for broker :in (brokers-of *server*))
                                                      (when (typep broker 'application)
                                                        (summing (hash-table-count (session-id->session-of broker))))))
       (format t "Time elapsed while rendering this response: ~,4F sec~%" (local-time:timestamp-difference (local-time:now) start))))))
